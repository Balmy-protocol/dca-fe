...
const OneClickMigrationModal = ({ open, onClose }: OneClickMigrationModalProps) => {
  ...
  const handleOnGoToStrategy = React.useCallback(
    (strategy: Strategy, token: Token, depositAmount: string) => {
      handleClose();
      // Give a little time for the modal to close
      setTimeout(() => {
        pushToHistory(
          `/earn/vaults/${strategy.network.chainId}/${strategy.id}?triggerSteps=true&assetToDeposit=${token.address}&assetToDepositAmount=${depositAmount}`
        );
