...
export default class AggregatorService {
  ...
  async getSwapOptionsPromise({
    ...
    console.log('got options', swapOptionsResponse);

    if (usePermit2) {
      return swapOptionsResponse as Record<string, Promise<EstimatedQuoteResponseWithTx>>;
    } else if (takerAddress) {
      console.log('got taker address so building swap options');
      const result: Record<string, Promise<QuoteResponseWithTx>> = {};
      ...
      for (const sourceId in quotes) {
        result[sourceId] = Promise.all([quotes[sourceId], txs[sourceId]]).then(([quote, tx]) => ({
          ...quote,
          tx,
        }));
      }
    ...
    return swapOptionsResponse as Record<string, Promise<EstimatedQuoteResponse>>;
  }
