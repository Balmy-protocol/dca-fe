// COPIED FROM https://raw.githubusercontent.com/kilkelly/redux-localstorage-simple/master/src/index.js
/* eslint-disable */
    ...
    function _save() {
      if (states.length === 0) {
        ...
        states.forEach((state) => {
          ...
          if (!isUndefined(stateForLocalStorage)) {
...
export function load({
  ...
  if (states.length === 0) {
    ...
    states.forEach(function (state) {
      ...
      if (!isUndefined(val)) {
        loadedState = mergeWith({}, loadedState, realiseObject(state, val), (objValue, srcValue) => {
          if (isArray(srcValue) || isObject(srcValue)) {
            return undefined;
          }
          return !isUndefined(srcValue) ? srcValue : objValue;
        });
      } else {
        warn_(
          "Invalid load '" +
