...
const CustomDot = <T extends { actions: DataItemAction[]; mode: 'light' | 'dark' }>(props: DotProps<T>) => {
  ...
  const isOnlyIncreased = !payload?.actions.some((action) => action.type !== EarnPositionActionType.INCREASED);
  const isOnlyWithdrew = !payload?.actions.some((action) => action.type !== EarnPositionActionType.WITHDREW);
